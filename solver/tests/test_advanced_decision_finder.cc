...
TEST_F(AdvancedDecisionFinderTest, LastDraw) {
  Hand my_hand(CardUtils::parseCards("Ac Ks"),
               CardUtils::parseCards("2c 2d 2s 3s 3d"),
               CardUtils::parseCards("9h 9d 9c Js"));

  Hand other_hand(CardUtils::parseCards("As Kc Qd"),
...
TEST_F(AdvancedDecisionFinderTest, GreatSet) {
  ...
  GameState game_state{my_hand, std::vector<Hand>(), my_pull,
std::vector<Card>()}; Decision d =
AdvancedDecisionFinder(evaluator).findBestDecision(game_state); std::cout <<
"best decision is: " << d;
}
/*
