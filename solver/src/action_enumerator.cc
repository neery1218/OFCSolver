...
vector<Decision> ActionEnumerator::findAllSetDecisionsHelper(const set<Card>& cards, const vector<Placement>& acc)
{
  vector<Decision> out;
...
vector<Decision> ActionEnumerator::findAllSetDecisions(const GameState& game_state)
{
  ...
  for (auto& d : decisions) {
    ...
    if (numTop <= 3 && numMid <= 5 && numBot <= 5 && (numNonFantasyCardsTop < 1 || game_state.other_hands.size() > 0 && numNonFantasyCardsTop < 2))
...
vector<Decision> ActionEnumerator::findAllDrawDecisions(const GameState& game_state)
{
