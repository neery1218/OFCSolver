...
Decision AdvancedDecisionFinder::findBestDecision(const GameState &game_state) {
  ...
  vector<Decision> top_n_decisions_stage_one =
      stageOneEvaluation(all_decisions, top_n, game_state, params);

  Decision best_decision =
      stageTwoEvaluation(top_n_decisions_stage_one, game_state, params);

  return best_decision;
}

vector<Decision> AdvancedDecisionFinder::stageOneEvaluation(
    const vector<Decision> &all_decisions, unsigned int n,
  ...
  for (int i = 0; i < all_decisions.size(); ++i) {
    ...
    futures.push_back(async(std::launch::async, [all_decisions, local_eval,
                                                 solver_params, game_state,
                                                 &initial_deck, dead_cards,
                                                 seeds, i]() {
      GameState new_state{
          game_state.my_hand.applyDecision(all_decisions[i]), vector<Hand>(),
      ...
      return AdvancedSolver(local_eval, seeds[i])
          .solve(solver_params.stage_one_iterations, new_state, initial_deck,
...
Decision AdvancedDecisionFinder::stageTwoEvaluation(
    const vector<Decision> &all_decisions, const GameState &game_state,
