...
/*
   ...
   &game_state) const { if (game_state.my_hand.size() == 5) return
   SolverParams{50, 100}; else if (game_state.my_hand.size() == 7) return
   SolverParams{50, 100}; else if (game_state.my_hand.size() == 9) return
   SolverParams{50, 100}; else if (game_state.my_hand.size() == 11) return
   SolverParams{50, 100}; else if (game_state.my_hand.size() == 13) return
   SolverParams{1, 1}; else throw runtime_error("Hand size is not valid!");
   }
   */
    ...
    boost::asio::post(
        pool, [all_decisions, local_eval, solver_params, game_state,
          ...
          GameState new_state{
              ...
              game_state.my_pull, dead_cards, game_state.n_solves,
...
AdvancedDecisionFinder::stageTwoEvaluation(
  ...
  for (int i = 0; i < all_decisions.size(); ++i) {
    for (int j = 0; j < split; ++j) {
      const FastPokerHandEvaluator *local_eval = evaluator;
      int num_iterations_split = game_state.n_solves / split;
